...
typedef union _RGBQUAD {
    COLORREF rgb;
    struct { BYTE b; BYTE g; BYTE r; BYTE Reserved; };
} _RGBQUAD, * PRGBQUAD;

typedef struct { FLOAT h; FLOAT s; FLOAT l; } HSL;

// ============================================================================
// FUNÇÕES DE EFEITOS (APENAS VISUAIS, SEM DESTRUIÇÃO)
// ============================================================================

DWORD WINAPI texts(LPVOID lpvd) {
    HDC dc = GetDC(NULL);
    const char* lpStrings[] = { "LOLMETAL", "SAFE VERSION", "NYAN CAT", "TESSERACT", "GDI EFFECTS", "BYTEBEAT" };
    while (true) {
        if (rand() % 25 == 24) {
...
DWORD WINAPI tesseract(LPVOID lpvd) {
    ...
    int cx = size, cy = size;
    int xdv = 10, ydv = 10;
    float angleX = 0.01, angleY = 0.01, angleZ = 0.01;
    int d = 60;
